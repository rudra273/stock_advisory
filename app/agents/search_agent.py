...
class SearchAgent(BaseAgent):
    ...
    def execute(self, query: str, **kwargs) -> Dict[str, Any]:
        """
        Google-only search for generic queries
        
        Args:
            query: Search query for any topic
            
        Returns:
            Dictionary with Google search analysis
        """
        results = {
            "query": query,
            "google_results": [],
        ...
        # 1. Get Google search results and scrape them
        google_results = self._get_google_news(query)
        results["google_results"] = google_results
        
        # 2. Create summary for Google articles with generic prompt
        if google_results:
            results["summary"] = self._create_generic_summary(google_results, query)
        else:
            results["summary"] = f"No relevant information found for '{query}'"
        
        # 3. Collect all sources in structured format
        source_info = self._collect_google_sources(results)
        results["sources"].update(source_info)
        
        return results

    def search_stock_news(self, query: str, use_google: bool = False, **kwargs) -> Dict[str, Any]:
        """
    ...
    def _create_generic_summary(self, google_results: List[Dict], query: str) -> str:
        """Create a generic summary for Google search results"""
        ...
        # Generic prompt for any topic
        prompt = f"""
        Summarize the following search results about "{query}":
        
        {combined_text}
        
        Create a comprehensive and informative summary covering the main points, key insights, 
        and important information found across all sources. Focus on providing a clear overview 
        of the topic based on the available information.
        """
        ...
        try:
            ...
            print(f"Error creating generic summary: {e}")
            return f"Summary of {len(google_results)} search results about {query}"
    ...
    def _collect_google_sources(self, results: Dict) -> Dict[str, List[str]]:
        """Collect sources specifically for Google-only search results"""
        urls = set()
        domains = set()
        api_sources = ["Google Custom Search API"]
        
        # Collect URLs and domains from Google results
        if results.get("google_results"):
            for item in results["google_results"]:
                url = item.get('url')
    ...
    def _collect_all_sources_structured(self, results: Dict, use_google: bool = False) -> Dict[str, List[str]]:
        ...
        Works for both search_stock_news() and search_generic_news() methods
        """
        urls = set()
        domains = set()
        api_sources = []
...
def search_stock_news(query: str, use_google: bool = False) -> Dict[str, Any]:
    ...
    """
    agent = SearchAgent()
    return agent.search_stock_news(query, use_google=use_google)
...
def search_google_only(query: str) -> Dict[str, Any]:
    """
    Google-only search for any topic
    
    Args:
        query: Search query for any topic
        
    Returns:
        Dictionary with Google search analysis including:
        - Google search results
        - Generic summary of all results
        - All source links used
    """
    agent = SearchAgent()
...
if __name__ == "__main__":
    ...
    print("\n=== Google-Only Search ===")
    google_results = search_google_only("artificial intelligence trends 2024")
